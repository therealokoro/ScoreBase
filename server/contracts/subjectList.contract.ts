...
export const create = oc
  .input(UpsertSubjectListSchema)
  .output(SubjectListSchema)
  .errors({ CONFLICT: { message: "A subject list with this name already exist" } })

export const update = oc
  .input(UpdateSubjectListSchema)
  .output(SubjectListSchema)
  .errors({
    NOT_FOUND: { message: "The subject list was not found" },
    CONFLICT: { message: "A subject list with this name already exist" }
  })
